...
2. Keep `sift-mcp` install and command path stable during transition.
3. Keep base install lean; heavy compute dependencies remain optional extras.

## Current State (February 19, 2026)

- Package name: `sift-mcp`
- Entrypoints:
  - `sift-mcp` (gateway/MCP flows)
...
- Keep `sift-mcp` as authoritative package.
...
- Publish `sift-data` as a thin wrapper distribution that depends on `sift-mcp`.
- Ensure `sift` command behavior is identical.
- Preserve all MCP behavior and docs under `sift-mcp`.

### Phase 3: Long-term branding

- Evaluate making `sift-data` primary distribution name.
- Keep `sift-mcp` as compatibility package for at least one major cycle.
...
- `uv build` succeeds.
- `sift` and `sift-mcp` console scripts are generated from package metadata.

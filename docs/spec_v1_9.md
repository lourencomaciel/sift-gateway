...
- Default import roots include stdlib helpers (`math`, `statistics`, `decimal`, `datetime`, `re`, `itertools`, `collections`, `functools`, `operator`, `heapq`, `json`, `csv`, `io`, `string`, `textwrap`) plus `jmespath`, `pandas`, and `numpy`. The `io` module restricts file-backed classes; only `StringIO` and `BytesIO` are usable.

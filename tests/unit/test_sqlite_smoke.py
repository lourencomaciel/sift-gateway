...
from mcp_artifact_gateway.db.backend import Dialect, SqliteBackend
        ...
        with sqlite_backend.connection() as conn:
            with pytest.raises(Exception):
                conn.execute(
                    ...
                    (
                        "local", "art-1", "nonexistent-session", "tool",
                        "upstream-1", "rk-1", "ph-1", "cv-1", 100, 0, 100, "v1",
                    ),
                )
    ...
    def test_json_column_roundtrip(self, sqlite_backend: SqliteBackend) -> None:
        ...
        with sqlite_backend.connection() as conn:
            ...
            conn.execute(
                ...
                (
                    "local", "ph-json", {"key": "value", "nested": [1, 2]},
                    "none", b"data", 4, "v1", 4, 0, 4,
                ),
            )
            conn.commit()
            row = conn.execute(
        ...
        with sqlite_backend.connection() as conn:
            ...
            for i in range(3):
                conn.execute(
                    ...
                    (
                        "local", f"art-{i}", "sess-seq", "tool", "up1",
                        f"rk-{i}", "ph-seq", "v1", 4, 0, 4, "v1",
                    ),
                )
            conn.commit()
            rows = conn.execute(

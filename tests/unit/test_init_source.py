...
def _touch_openclaw(path: Path) -> None:
    """Write a minimal OpenClaw provider.mcpServers config."""
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(
        '{"provider":{"mcpServers":{"dummy":{"command":"npx"}}}}',
        encoding="utf-8",
    )


def test_resolve_source_shortcut_openclaw_macos(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setattr(
    ...
    source = tmp_path / ".openclaw" / "openclaw.json"
    _touch_openclaw(source)

    resolved = resolve_source_shortcut("openclaw")

    assert resolved == source.resolve()


def test_resolve_source_shortcut_openclaw_linux(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setattr(
...
def test_openclaw_aliases_resolve(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setattr(
    ...
    for alias in ("open-claw", "open_claw", "OpenClaw"):
        resolved = resolve_source_shortcut(alias)
        assert resolved == source.resolve()


def test_auto_includes_openclaw(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setattr(
    ...
    openclaw = tmp_path / ".openclaw" / "openclaw.json"
    _touch_openclaw(openclaw)

    matches, _checked = find_source_shortcut_matches("auto", cwd=tmp_path)

    resolved = {str(path) for path in matches}
    assert str(openclaw.resolve()) in resolved

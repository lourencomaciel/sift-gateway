...
from sift_mcp.core.runtime import ArtifactCaptureRuntime
...
def execute_artifact_capture(
    ...
    *,
    ...
    if (
        not isinstance(upstream_instance_id, str)
        or not upstream_instance_id
    ):
    ...
    if (
        not isinstance(request_args_prefix, str)
        or not request_args_prefix
    ):

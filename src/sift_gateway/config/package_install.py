...
def _filter_shared_roots(
    ...
    for spec in packages:
        uninstalling.add(_strip_spec(spec).lower().replace("-", "_"))
...
def _update_allowlist_by_roots(
    ...
    current: list[str] | None = cfg.get("code_query_allowed_import_roots")
    if current is None:
        return
    ...
    if add:
        for root in roots:
            if root not in effective:
                ...
                effective.add(root)
                changed = True
                print(f"  Added '{root}' to code query allowlist.")
    else:

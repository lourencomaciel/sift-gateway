...
Merge environment variables (``SIFT_GATEWAY_*`` prefix), an optional
...
class GatewayConfig(BaseSettings):
    ...
    Merge environment variables (``SIFT_GATEWAY_*`` prefix), an
    ...
    data_dir: Path = Field(
        ...
        description="Root data directory (default .sift-gateway/)",
    )
    ...
    code_query_max_bytes_out: int = Field(
        200_000,
        ge=1,
        description=(
            "Maximum serialized response bytes for query_kind=code "
            "before schema_ref mode is returned."
        ),
    )
    ...
    @classmethod
    def _validate_secret_redaction_placeholder(cls, value: str) -> str:
...
def _env_path_segments(env_key: str) -> tuple[str, ...] | None:
    """Extract path segments from an SIFT_GATEWAY_ env key.

    Strip the ``SIFT_GATEWAY_`` prefix and split on ``__``
...
def _load_env_overrides() -> dict[str, object]:
    """Collect and normalize SIFT_GATEWAY_* env var overrides.

    Iterate all environment variables, filter for the
    ``SIFT_GATEWAY_`` prefix, normalize segments, coerce
...
def _check_legacy_upstreams(from_file: dict[str, object]) -> None:
    ...
    if has_upstreams and not uses_new:
        msg = (
            ...
            "Run 'sift-gateway init --from <config>' "
            "to migrate."
        )
        raise ValueError(msg)

...
Merge environment variables (``SIDEPOUCH_MCP_*`` prefix), an optional
...
class GatewayConfig(BaseSettings):
    """Root configuration for the SidePouch.

    Merge environment variables (``SIDEPOUCH_MCP_*`` prefix), an
    ...
    data_dir: Path = Field(
        ...
        description="Root data directory (default .sidepouch-mcp/)",
    )
    ...
    postgres_dsn: str = Field(
        "postgresql://localhost:5432/sidepouch",
...
def _env_path_segments(env_key: str) -> tuple[str, ...] | None:
    """Extract path segments from an SIDEPOUCH_MCP_ env key.

    Strip the ``SIDEPOUCH_MCP_`` prefix and split on ``__``
...
def _load_env_overrides() -> dict[str, object]:
    """Collect and normalize SIDEPOUCH_MCP_* env var overrides.

    Iterate all environment variables, filter for the
    ``SIDEPOUCH_MCP_`` prefix, normalize segments, coerce

...
def build_question_metrics(
    *,
    ...
    Args:
        ...
        condition: Condition label (``"sift"`` or ``"baseline"``).
...
def build_baseline_metrics(
    *,
    ...
    """Build per-question metrics for the baseline condition.

    The baseline has no agent loop, so agent-specific fields
    (turns, tool_calls, code_query_*) are zeroed out.
    ...
    Args:
        ...
        truncated: Whether the dataset was truncated to fit.

    Returns:
        Dict with per-question metrics (baseline condition).
    """
    return {
        "condition": "baseline",
        "question_id": question_id,
...
def build_report(
    ...
    *,
    ...
    # Add per-condition breakdown when both conditions are present.
    conditions = {r.get("condition", "sift") for r in results}
    if len(conditions) > 1:
        per_condition: dict[str, dict[str, Any]] = {}
        for cond in sorted(conditions):
            cond_results = [
                r for r in results if r.get("condition", "sift") == cond
            ]
            c_total = len(cond_results)
            c_correct = sum(1 for r in cond_results if r.get("correct"))
            c_input = sum(r.get("input_tokens", 0) for r in cond_results)
            c_output = sum(r.get("output_tokens", 0) for r in cond_results)
            c_latencies = [
                r["latency_ms"]
            ...
            per_condition[cond] = {
                "accuracy": f"{c_correct}/{c_total}",
                "accuracy_pct": (
                    round(c_correct / c_total * 100, 1) if c_total else 0
                ),
